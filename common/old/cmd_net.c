...
{
puts("\n\r");
puts("!!!!!!!START Preparation for RECIEVE and TRANSMIT  TSEC and PHY!!!!\n\r");
...
int do_ping (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    */
	//NetPingIP=0xC0A8821C;
	NetLoop(PING);
	
	/*
	if (NetLoop(PING) < 0)
	{
		//printf("ping failed; host %s is not alive_BAD_BOY++++\n", argv[1]);
		puts("PING FAILED\n\r"); 
		return 1;
	}
    */
	return 1;
	//printf("host %s is alive_VERY_GOOD++++++!\n", argv[1]);
	puts("PING SUCCESS COMPLETE\n\r");	
	//return 0;
}
