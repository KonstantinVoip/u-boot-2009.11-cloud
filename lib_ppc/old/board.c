...
{
	puts ("I2C:   ");
	//puts ("Current non Initialize I2C\r\n");
	//puts ("++++++++++board.c/init_func_i2c++++++++++++");
	//
...
*/
	...
	//IP Address //
	bd->bi_ip_addr = 0xC0A8827C;//192.168.130.124 //getenv_IPaddr ("ipaddr");
...
*/
	...
	//kosta_cmd_run();
    //P2020 for mpc _LocalBus_Initialize  <-> Cyclone 3
      puts("mpc _LocalBus_Initialize\n\r"); 
	  flash_size = flash_init ();
	  bd->bi_flashstart = CONFIG_SYS_FLASH_BASE;	// update start of FLASH memory 
	  bd->bi_flashsize = flash_size;	// size of FLASH memory (final value) 
	  printf("bd->bi_flashstart =%x,bd->bi_flashsize=%x\n\r",bd->bi_flashstart,bd->bi_flashsize );
