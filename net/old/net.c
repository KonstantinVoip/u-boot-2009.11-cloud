...
static void NetInitLoop(proto_t protocol)
{
    ...
    puts("+++++++++net.c/NetInitLoop+++++++\n\r"); 
	/* update only when the environment has changed */
	if (env_changed_id != env_id)
	{
	  	NetOurIP=0xC0A8827C;           //192.168.130.124
		NetOurGatewayIP =0xC0A8827C;    //192.168.130.124
		NetOurSubnetMask=0xFFFF0000;    //255.255.0.0
		NetServerIP =0xC0A88280;        //192.168.130.128
...
puts("+++++++++net.c/NetMAINLoop+++++++\n\r"); 	
/*	
puts("++++++++++++set MAC ethaddr+++++++++++++\n\r");
//MAC=21:21:41:41:01:41 
uchar *p="!!AAccddeeff"; 
memcpy (NetOurEther, p, 6);
*/
...
int PingSend(void)
{
	...
	ip->ip_p     = 0x01;		/* ICMP */
	ip->ip_sum   = 0;
	ip->ip_src   = 0xC0A8827C; //192.168.130.124 //source address
	ip->ip_dst   = 0xC0A8821C; //192.168.130.28  //destination address
...
int NetSetEther(volatile uchar * xet, uchar * addr, uint prot)
{
	...
	memcpy (et->et_dest, addr, 6);  //kosta_comment
