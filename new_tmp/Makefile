...
SRAM_BOOT = sram_boot
ifeq ($(CONFIG_RAMBOOT_SPIFLASH),y)
U_BOOT_ESPI = $(obj)u-boot-espi.bin
endif
ifeq ($(CONFIG_RAMBOOT_SDCARD),y)
U_BOOT_SD = $(obj)u-boot-sd.bin
endif
endif
...
$(I2C_SPL):	$(TIMESTAMP_FILE) $(VERSION_FILE) $(obj)include/autoconf.mk
		$(MAKE) -C i2c_spl/board/$(BOARDDIR) clean all
else
ALL += $(obj)u-boot.srec $(obj)u-boot.bin $(obj)System.map $(U_BOOT_NAND) $(U_BOOT_ONENAND) $(U_BOOT_NAND_SRAM) $(U_BOOT_ESPI) $(U_BOOT_SD)
...
$(SRAM_BOOT):	$(TIMESTAMP_FILE) $(VERSION_FILE) $(obj)include/autoconf.mk
		$(MAKE) -C cpu/$(CPU)/sram_boot all

$(U_BOOT_NAND_SRAM): $(NAND_SPL) $(SRAM_BOOT) $(obj)u-boot.bin
		cat $(obj)nand_spl/u-boot-spl-16k.bin $(obj)cpu/$(CPU)/u-boot-sram.bin $(obj)u-boot.bin > $(obj)u-boot-nand.bin

$(U_BOOT_ESPI): $(SRAM_BOOT) $(obj)u-boot.bin
		cat $(obj)cpu/$(CPU)/u-boot-sram.bin $(obj)u-boot.bin > $(obj)u-boot-espi.bin

$(U_BOOT_SD): $(SRAM_BOOT) $(obj)u-boot.bin
		cat $(obj)cpu/$(CPU)/u-boot-sram.bin $(obj)u-boot.bin > $(obj)u-boot-sd.bin
...
P2020DS_SDCARD_config \
P2020DS_SPIFLASH_config \
P2020DS_DDR2_config \
...
# For list of make targets for P1/P2 RDB refer to doc/README.p1_p2_rdb
P1020RDB%config \
P1011RDB%config \
P2020RDB%config \
P2010RDB%config:	unconfig
	@$(MKCONFIG) -t $(@:_config=) P1_P2_RDB ppc mpc85xx p1_p2_rdb freescale

P1021MDS_config \
P1021MDS_NAND_config:		unconfig
	@$(MKCONFIG) -t $(@:_config=) P1021MDS  ppc mpc85xx p1021mds freescale
...
ifeq ($(CONFIG_I2C_U_BOOT),y)
clean:	$(TIMESTAMP_FILE) $(VERSION_FILE) $(obj)include/autoconf.mk
	$(MAKE) -C i2c_spl/board/$(BOARDDIR) clean
else
clean:
	...
	@rm -f $(obj)cpu/$(CPU)/{u-boot-sram,u-boot-sram.map}
...
clobber:	clean
	...
	@[ ! -d $(obj)cpu/$(CPU) ] || find $(obj)cpu/$(CPU) -name "*" -type l -print | xargs rm -f
