...
static inline void * word_flash_map (u16 offset)
{
...
/*-----------------------------------------------------------------------
 First Flash Initialization Functions on CYCLONE 3 PLIS
...
//Testig for Local Bus is Enable
 __flash_write16(testlbc_value, word_flash_map(l_adress_read0));


 printf("----------------------------------------------\n");
 dannie0=__flash_read16(word_flash_map(l_adress_read0));
 if(dannie0==0x00AA)
 {
 printf("Local_BUS=OK_%x\n\r",dannie0);
 dannie2= __flash_read16(word_flash_map(l_adress_read2));
 dannie4= __flash_read16(word_flash_map(l_adress_read4));
 dannie6= __flash_read16(word_flash_map(l_adress_read6));
 dannie8= __flash_read16(word_flash_map(l_adress_read8));
 if(dannie8==0x7FE)
 {
 Rx_bufLength=2048;
 }
 ...
 {
 Rx_bufLength=dannie8;
 }
 
 
 dannie10=__flash_read16(word_flash_map(l_adress_read10));
 if(dannie10==0x7FE)
 {
 Tx_bufLength=2048;
 }
 ...
 printf("02_TYPE         =    0x%08X\n\r",dannie2);
 printf("04_Version      =    0x%08X\n\r",dannie4);
 printf("06_Version FPGA =    0x%08X\n\r",dannie6);
 printf("08_MaxRx_buf    =    %04d\n\r",Rx_bufLength);
 printf("10_MaxTx_buf    =    %04d\n\r",Tx_bufLength);
 status=1;
 }
 else
 {
 printf("?????/Local_BUS=ERROR???????\n\r");
 printf("0_Rgister      =  0x%x\n\r",dannie0);
 status=0;
