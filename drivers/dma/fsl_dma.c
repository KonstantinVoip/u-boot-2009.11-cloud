...
int dmacpy(phys_addr_t dest, phys_addr_t src, phys_size_t count) {
	...
	while (count) {
		...
		out_dma32(&dma->dar, (uint) (dest & 0xFFFFFFFF));
		out_dma32(&dma->sar, (uint) (src & 0xFFFFFFFF));
		out_dma32(&dma->satr, in_dma32(&dma->satr) | (uint)(src >> 32));
		out_dma32(&dma->datr, in_dma32(&dma->datr) | (uint)(dest >> 32));
